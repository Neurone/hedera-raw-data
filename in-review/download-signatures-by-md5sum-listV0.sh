...
VALIDATOR_SIGNATURE_DESTINATION_FOLDER="$VALIDATOR_SIGNATURES_FOLDER/$DAY"
...
echo "$(print_timestamp) ⛶ Validator's signatures destination ...: $VALIDATOR_SIGNATURE_DESTINATION_FOLDER"

init_working_folders

create_folder_if_not_present $VALIDATOR_SIGNATURE_DESTINATION_FOLDER

#TODO check parameters
#   file sorgente esiste

# verifica se ci sono già dei file scaricati, nel caso riparte da lì
LAST_SIGNATURE_FILE=$(find $VALIDATOR_SIGNATURE_DESTINATION_FOLDER -type f -name "$DAYT*" -printf "%f\n" | sort | tail -n1)

if [ -z "$LAST_SIGNATURE_FILE" ]; then
    # Start from the top of the list
    echo "$(print_timestamp) ☕ Downloading signature files from S3 record folder for validator $VALIDATOR_ID on $DAY"
    cat $VALIDATOR_FILE_LIST_FOLDER/$DAY$HD_SIGNATURES_LIST_MD5_EXTENSION |\
        cut -d' ' -f2 |\
        ...
        #xargs -I {} aws s3 cp s3://hedera-mainnet-streams/$VALIDATOR_S3_SOURCE_FOLDER/{} $VALIDATOR_SIGNATURE_DESTINATION_FOLDER --request-payer requester --no-paginate --output text
else
    ...
    echo "$(print_timestamp) ☕ Continuing download of signature files from S3 record folder for validator $VALIDATOR_ID on $DAY, starting from $LAST_SIGNATURE_FILE"
    cat $VALIDATOR_FILE_LIST_FOLDER/$DAY$HD_SIGNATURES_LIST_MD5_EXTENSION |\
        cut -d' ' -f2 |\
...
check_md5sum_list_over_folder $VALIDATOR_RECORD_DESTINATION_FOLDER $VALIDATOR_FILE_LIST_FOLDER/$DAY$HD_RECORDS_LIST_MD5_EXTENSION
